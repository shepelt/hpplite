...
HppliteCommitment *hpplite_commitment_new(
  ...
  HppliteCommitment *pCommit = sqlite3_malloc(sizeof(HppliteCommitment));
  if (pCommit) {
    memset(pCommit, 0, sizeof(HppliteCommitment));
    pCommit->height = height;
    memcpy(pCommit->stateRoot, pStateRoot, HPPLITE_HASH_SIZE);
    pCommit->zBatchRef = sqlite3_mprintf("%s", zBatchRef);
    pCommit->timestamp = (uint64_t)time(NULL);
  }
  return pCommit;
}
...
void hpplite_commitment_free(HppliteCommitment *pCommit) {
  if (pCommit) {
    sqlite3_free(pCommit->zBatchRef);
    sqlite3_free(pCommit);
  }
}

/*
** Serialize commitment to JSON (single line for JSONL)
*/
char *hpplite_commitment_to_json(const HppliteCommitment *pCommit) {
  char hexBuf[HPPLITE_HASH_SIZE * 2 + 1];
  char *escaped;

  if (!pCommit) return NULL;

  hpplite_to_hex(pCommit->stateRoot, HPPLITE_HASH_SIZE, hexBuf);
  escaped = json_escape_string(pCommit->zBatchRef ? pCommit->zBatchRef : "");

  char *result = sqlite3_mprintf(
    "{\"height\":%llu,\"state_root\":\"%s\",\"batch_ref\":\"%s\",\"timestamp\":%llu}",
    (unsigned long long)pCommit->height,
    hexBuf,
    escaped ? escaped : "",
    (unsigned long long)pCommit->timestamp
  );
...
/*
** Parse commitment from JSON
*/
HppliteCommitment *hpplite_commitment_from_json(const char *zJson) {
  HppliteCommitment *pCommit = NULL;
  const char *p = zJson;
  char *zKey, *zVal;

  if (!zJson) return NULL;

  pCommit = sqlite3_malloc(sizeof(HppliteCommitment));
  if (!pCommit) return NULL;
  memset(pCommit, 0, sizeof(HppliteCommitment));
  ...
  while (*p) {
    ...
    if (strcmp(zKey, "height") == 0) {
      pCommit->height = (uint64_t)json_parse_number(&p);
    } else if (strcmp(zKey, "timestamp") == 0) {
      pCommit->timestamp = (uint64_t)json_parse_number(&p);
    } else if (strcmp(zKey, "state_root") == 0) {
      zVal = json_parse_string(&p);
      if (zVal) {
        hpplite_from_hex(zVal, pCommit->stateRoot, HPPLITE_HASH_SIZE);
        sqlite3_free(zVal);
      }
    } else if (strcmp(zKey, "batch_ref") == 0) {
      pCommit->zBatchRef = json_parse_string(&p);
    } else {
      if (*p == '"') {
        zVal = json_parse_string(&p);
        sqlite3_free(zVal);
